...
// console.log(http.readyState);       // O state  

/* (3) Open the request, Get type url from documentation and set async = true */
http.open('GET', `http://localhost:8080/lms/api/members?size=${pageSize}&page=${page}&q=${query}`, true);

/* (4) Set additional informations (from documentation we have to find it) */
...
function initPagination(totalMembers) {
    ...
    html = `
        ...
        `;
    $('#pagination > .pagination').html(html).click((eventData)=>{
